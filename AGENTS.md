...
- End-user projects install `@agent-dev-lab/runtime`; the CLI loads it from the target project's `node_modules`.
- SQLite database is auto-created at `.data/agent-dev-lab.sqlite` on first access — configurable via `ADL_SQLITE_PATH`.
- `apps/docs` gitignores `src/content/docs/api/`; generated by starlight-typedoc during `astro dev`, `astro check`, or `astro build`.
