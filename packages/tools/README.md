...
On providers, omitted `allowWrite` / `allowRead` default to `[cwd]`. Pass `allowWrite: []` for a sandbox that can run commands but write nowhere; `allowRead: []` (or `null` on the file jail) allows no reads. Pass `UNBOUNDED_ALLOW_READ` / bash `null` for host-wide reads. Escape-hatch executors still require `allowWrite` at construction; omitted `allowRead` defaults to `allowWrite`. Sandboxed commands inherit no host environment by default — pass `allowEnv: true` or a name/glob/`RegExp` allowlist.
