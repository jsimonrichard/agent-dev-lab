...
| Overclaiming maturity — cancellation is partial, mid-closure resume does not exist    | Scope claims to what is built (attempt-lineage retry + path-stable skip, dual stores, event-sourced inspection). A limitations section that names Temporal-class durability / crash checkpoints as out of scope. |
