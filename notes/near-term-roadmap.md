...
| Structural cleanup                              | [`structural-cleanup.md`](./structural-cleanup.md) | After this publish. Split the tree panel without dropping the INP memos, stop drilling `showErrors` through the JSON editor, and shorten the sandboxing note to the threat model plus open decisions. `WorkflowStore`'s method growth is the revisit in [`retry-side-effects.md`](./retry-side-effects.md). |
