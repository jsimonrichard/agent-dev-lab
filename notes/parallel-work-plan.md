...
| Item                             | Detail                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`AGENTS.md:45` is stale**      | Said CI "runs lint and format checks". **Fixed 2026-09-10** — replaced with both jobs and every step, in `ci.yml` order.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **`watch.e2e.test.ts` is flaky** | Timed out at 29s on the PR #31 merge commit with `generation: 0` and `lastReloadError: null` — `reload()` was never called. **Fixed 2026-09-10 — see [`watch-e2e-flake.md`](./watch-e2e-flake.md).** Bun 1.3.13's `fs.watch` collapsed an atomic temp+rename save to one event and fired nothing on a per-file watch, blinding chokidar and so Vite's watcher; `watchAdlProject` survived only by treating the temp file as the source. Bun is now 1.4.2, `watchAdlProject` uses chokidar, and the Vite reload plugin is deleted — one watcher, one reload per save. Still open: the `packages/core/dist` failure, and CI's intermittency (mechanism explained, timing not). |
| **Bun `node:test` cascade**      | An async throw in a `node:test`-style file under `bun test` took an unrelated file's whole suite down with it. **Closed 2026-09-10 — see [`bun-node-test-cascade.md`](./bun-node-test-cascade.md).** Not oven-sh/bun#5090 (the 2023 feature request Bun's error text points at); the matching issue is #23077. Nothing filed — it does not reproduce on Bun 1.4, and the repo is now on 1.4.2.                                                                                                                                                                                                                                                                               |
| **PR #32 branch naming**         | The two follow-up commits sit on `ci-install-sandbox-deps`, reusing already-merged PR #31's branch name. **Skipped 2026-09-10**, as the item allows: both PRs are merged and the branch is deleted upstream, so correcting it would mean rewriting merged history for a cosmetic gain.                                                                                                                                                                                                                                                                                                                                                                                       |
