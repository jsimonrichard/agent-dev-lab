...
| **`watch.e2e.test.ts` is flaky** | "GET /api/project picks up an atomic edit" (line 232) timed out at 29s on the PR #31 merge commit and passed on a re-run of that identical commit. The failure showed `generation: 0` with `lastReloadError: null` — `reload()` was never called, so this is file-watch reliability, not a too-short poll. **Diagnosed 2026-09-10 — see [`watch-e2e-flake.md`](./watch-e2e-flake.md).** Bun 1.3.13's `fs.watch` collapses an atomic temp+rename save to a single event and fires nothing on a per-file watch, which blinds Vite's chokidar; `watchAdlProject` survives it only because it treats the temp file as the source. Fixed in Bun 1.4. The dead `ADL_VITE_PROJECT_WATCH` hand-off is removed; a second, deterministic failure of the same test appears whenever `packages/core/dist` is absent. |
| **Bun `node:test` cascade**      | An async throw in a `node:test`-style file under `bun test` corrupts the root context and takes an unrelated file's whole suite down with it. It is **loud** (Bun counts the errored file as a failure and exits non-zero), so no guard is needed. **Closed 2026-09-08 — see [`bun-node-test-cascade.md`](./bun-node-test-cascade.md).** Not oven-sh/bun#5090 (that is the 2023 feature request Bun's error text points at); the matching issue is #23077. Nothing filed: it does not reproduce on Bun 1.4.0, and this repo is pinned to 1.3.13, so the follow-up is the Bun upgrade.                                                                                                                                                                                                                    |
