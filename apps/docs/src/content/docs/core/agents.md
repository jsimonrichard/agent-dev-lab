...
The runner emits run events via `RunRecorder`: `agent_started`, `agent_text_delta`, `agent_messages_committed`, `agent_finished`, `agent_failed`. See [Observability](/core/observability/) and [Streaming](/core/streaming/).
