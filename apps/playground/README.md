...
| Sandboxed workspace tools (`@agent-dev-lab/tools`)    | `src/tools/sandbox.ts` + `sandbox-demo` workflow (pooled `createWorkspaceToolProvider` with `backend: "asrt"` / `"native"`, per-run `cwd` via `toolProviderContext`, an AI-based `bash-safety-check` workflow)                                                |
...
| Retry copies a shared scope                           | `src/workflows/copy-memory.ts` (`fact-keeper` remembers a fact, then recalls it on the same `memoryScopeWithSuffix`; retry from recall copies the transcript)                                                                                                 |
